...
\begin{center}
\begin{tikzpicture}[scale=0.8,every node/.style={draw,minimum size=6mm,inner sep=1pt}]
  % datele A[0..8] with increased horizontal spacing (factor 1.5)
  \foreach \i/\v in {0/3,1/9,2/2,3/5,4/7,5/34,6/6,7/11,8/8}{
    \node (a\i) at (1.5*\i,0) {\(\v\)};
  }
  % linii de separare pentru blocuri de B=3 (positions scaled by 1.5)
  \draw[dashed] (4.5,-0.5) -- (4.5,0.5);
  \draw[dashed] (9,-0.5) -- (9,0.5);
  % etichete pentru blocuri
  \node at (1.5,-1) {bloc 0};
  \node at (6,-1) {bloc 1};
  \node at (10.5,-1) {bloc 2};
  % valorile M[b]
  \node (m0) at (1.5,-2) {\(\max\{3,9,2\}=9\)};
  \node (m1) at (6,-2) {\(\max\{5,7,34\}=34\)};
  \node (m2) at (10.5,-2) {\(\max\{6,11,8\}=11\)};
\end{tikzpicture}
\end{center}
